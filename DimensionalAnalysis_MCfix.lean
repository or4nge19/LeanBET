...
class has_time (α : Type u) where
  [dec : DecidableEq α]
  time : α

class has_length (α : Type u) where
  [dec : DecidableEq α]
  length : α

class has_mass (α : Type u) where
  [dec : DecidableEq α]
  mass : α

class has_amount_of_substance (α : Type u) where
  [dec : DecidableEq α]
  amount_of_substance : α

class has_electric_current (α : Type u) where
  [dec : DecidableEq α]
  electric_current : α

class has_temperature (α : Type u) where
  [dec : DecidableEq α]
  temperature : α

class has_luminous_intensity (α : Type u) where
  [dec : DecidableEq α]
...
def system1Perm : Fin 2 → system1
| (0 : Fin 2) => system1.length
| (1 : Fin 2) => system1.time

lemma system1Perm_zero : system1Perm 0 = system1.length := rfl
lemma system1Perm_one : system1Perm 1 = system1.time := rfl
...
example : (dimensional_matrix ![length system1, velocity system1, time system1] system1Perm)
  ...
  cases' h1 with h1 h1
  · cases' h2 with h2 h2'
    · simp [h1, h2, system1Perm_zero]
      rw [system1_length_to_has_length]
    · cases' h2' with h2 h2'
      · simp [h1, h2, system1Perm_zero]
        rw [system1_length_to_has_length]
        simp [velocity, time, length]
        rw [system1Perm_zero, system1_length_to_has_length]
        simp
      · simp [h1, h2', system1Perm_zero]
        rw [system1_length_to_has_length]
        simp [velocity, time, length]
  ...
  · subst h1
    ...
    · subst h
      simp_all only [Fin.isValue, Matrix.cons_val_zero, system1Perm_one, system1_time_to_has_time]
      apply Eq.refl
    · rcases h_1 with ⟨h⟩ | ⟨h_2⟩
      · subst h
        simp_all only [Fin.isValue, Matrix.cons_val_one, Matrix.head_cons, system1Perm_one,
          system1_time_to_has_time]
        simp [velocity, length, time, system1_time_to_has_time, system1Perm_one]
      · subst h_2
        simp_all only [Fin.isValue, Matrix.cons_val_two, Nat.succ_eq_add_one, Nat.reduceAdd,
          Matrix.tail_cons, Matrix.head_cons, system1Perm_one, system1_time_to_has_time]
        apply Eq.refl
  ...
  !![(1 : ℚ), 1; 0, -1] := by
    ...
    · cases' h2 with h2 h2
      · simp [h1, h2, system1Perm_zero]
        rw [system1_length_to_has_length]
        simp
      · simp [h1, h2, velocity, length, time, system1Perm_zero]
        rw [system1_length_to_has_length]
        simp
    · simp only [Matrix.of_apply, Matrix.cons_val', Matrix.empty_val', Matrix.cons_val_fin_one]
      ...
      cases' h2 with h2 h2
      · simp [h2, system1Perm_one]
        rw [system1_time_to_has_time]
        simp
      · simp [h2, velocity, length, time, system1Perm_one]
        rw [system1_time_to_has_time]
        simp
...
example : (dimension.velocity system1) system1.time = -1 := by
  simp [velocity, length, time, system1_time_to_has_time, system1_length_to_has_length]
